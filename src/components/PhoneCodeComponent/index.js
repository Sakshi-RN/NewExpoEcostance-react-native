...
const PhoneCodeComponent = ({ isVisible, toggleModal, onSelectPhoneCode }) => {
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity onPress={() => {
                onSelectPhoneCode && onSelectPhoneCode(item.phonecode);
                toggleModal();
              }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', padding: 10 }}>
                  <Text style={{ fontSize: 18,color:'black' }}>{item.phoneCodeWithFlag}</Text>
                </View>
              </TouchableOpacity>
            )}
          />
